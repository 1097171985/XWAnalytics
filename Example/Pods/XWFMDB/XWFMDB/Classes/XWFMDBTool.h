 ...
 */
#ifdef DEBUG
#define xw_log(...) NSLog(__VA_ARGS__)
#else
#define xw_log(...)
#endif

#define xw_completeBlock(obj) !complete?:complete(obj);

#define XW @"XW_"
#define xw_tableNameKey @"xw_tableName"
#define xw_rowid @"rowid"

#define xw_uniqueKeysSelector NSSelectorFromString(@"xw_uniqueKeys")
#define xw_ignoreKeysSelector NSSelectorFromString(@"xw_ignoreKeys")
#define xw_unionPrimaryKeysSelector NSSelectorFromString(@"xw_unionPrimaryKeys")

typedef NS_ENUM(NSInteger,xw_getModelInfoType){//过滤数据类型
    xw_ModelInfoInsert,//插入过滤
    xw_ModelInfoSingleUpdate,//单条更新过滤
    xw_ModelInfoArrayUpdate,//批量更新过滤
    xw_ModelInfoNone//无过滤
};
 ...
 */
+(id _Nonnull)xw_objectWithClass:(__unsafe_unretained _Nonnull Class)cla value:(id _Nonnull)value;
/**
 模型转字典.
 */
+(NSMutableDictionary* _Nonnull)xw_keyValuesWithObject:(id _Nonnull)object ignoredKeys:(NSArray* _Nullable)ignoredKeys;
/**
 判断并执行类方法.
 */
...
/**
 过滤建表的key.
 */
+(NSArray* _Nonnull)xw_filtCreateKeys:(NSArray* _Nonnull)createkeys ignoredkeys:(NSArray* _Nonnull)ignoredkeys;
/**
 如果表格不存在就新建.
 */
