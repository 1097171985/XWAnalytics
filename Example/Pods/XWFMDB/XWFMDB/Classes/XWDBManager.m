...
/**
 默认数据库名称
 */
#define SQLITE_NAME @"XWFMDB.db"

#define xw_debug(param) do{\
if(self.debug){xw_log(@"调试输出: %@",param);}\
}while(0)
...
static const void * const XWFMDBDispatchQueueSpecificKey = &XWFMDBDispatchQueueSpecificKey;

@interface XWDBManager()
/**
 数据库队列
 */
...
static XWDBManager* XWdb = nil;

@implementation XWDBManager
...
-(dispatch_queue_t)mulThreadPoolQueue{
    if (_mulThreadPoolQueue == nil) {
        _mulThreadPoolQueue = dispatch_queue_create("XWFMDB.excuteThreadPool", DISPATCH_QUEUE_SERIAL);
    }
...
/**
 注册数据变化监听.
 */
-(BOOL)registerChangeWithName:(NSString* const _Nonnull)name block:(xw_changeBlock)block{
...
-(void)doChangeWithName:(NSString* const _Nonnull)name flag:(BOOL)flag state:(xw_changeState)state{
...
/**
 数据库中是否存在表.
 */
-(void)isExistWithTableName:(NSString* _Nonnull)name complete:(xw_complete_B)complete{
...
/**
 对用户暴露的
 */
-(BOOL)xw_isExistWithTableName:( NSString* _Nonnull)name{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    NSAssert(name,@"表名不能为空!");
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db) {
...
/**
 创建表(如果存在则不创建).
 */
-(void)createTableWithTableName:(NSString* _Nonnull)name keys:(NSArray<NSString*>* _Nonnull)keys unionPrimaryKeys:(NSArray* _Nullable)unionPrimaryKeys uniqueKeys:(NSArray* _Nullable)uniqueKeys complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
    ...
    [self executeDB:^(FMDatabase * _Nonnull db) {
        ...
        for(int i=0;i<keys.count;i++){
            ...
            if(tempUniqueKeys.count && [tempUniqueKeys containsObject:key]){
                for(NSString* uniqueKey in tempUniqueKeys){
                    if([XWFMDBTool isUniqueKey:uniqueKey with:keys[i]]){
                        [sql appendFormat:@"%@ unique",[XWFMDBTool keyAndType:keys[i]]];
                ...
                if ([key isEqualToString:xw_primaryKey] && !unionPrimaryKeys.count){
                    [sql appendFormat:@"%@ primary key autoincrement",[XWFMDBTool keyAndType:keys[i]]];
                }else{
                    [sql appendString:[XWFMDBTool keyAndType:keys[i]]];
                }
            }
            ...
            if (i == (keys.count-1)) {
                if(unionPrimaryKeys.count){
                    ...
                    [unionPrimaryKeys enumerateObjectsUsingBlock:^(id  _Nonnull unionKey, NSUInteger idx, BOOL * _Nonnull stop) {
                        if(idx == 0){
                            [sql appendString:xw_sqlKey(unionKey)];
                        }else{
                            [sql appendFormat:@",%@",xw_sqlKey(unionKey)];
                        }
                    }];
                    [sql appendString:@")"];
                }
        ...
        if(uniqueKeys.count){
            NSAssert(!uniqueKeyFlag,@"没有找到设置的'唯一约束',请检查模型类.m文件的xw_uniqueKeys函数返回值是否正确!");
        }
...
/**
 插入数据.
 */
-(void)insertIntoTableName:(NSString* _Nonnull)name Dict:(NSDictionary* _Nonnull)dict complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
    NSAssert(dict,@"插入值字典不能为空!");
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db) {
        NSArray* keys = dict.allKeys;
        if([keys containsObject:xw_sqlKey(xw_primaryKey)]){
            NSInteger num = [self getKeyMaxForTable:name key:xw_sqlKey(xw_primaryKey) db:db];
            [dict setValue:@(num+1) forKey:xw_sqlKey(xw_primaryKey)];
        }
    ...
    //数据监听执行函数
    [self doChangeWithName:name flag:result state:xw_insert];
    xw_completeBlock(result);
}
/**
 批量插入
 */
-(void)insertIntoTableName:(NSString* _Nonnull)name DictArray:(NSArray<NSDictionary*>* _Nonnull)dictArray complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db) {
        [db beginTransaction];
...
/**
 ...
 over
 */
-(void)updateSetTableName:(NSString* _Nonnull)name class:(__unsafe_unretained _Nonnull Class)cla DictArray:(NSArray<NSDictionary*>* _Nonnull)dictArray complete:(xw_complete_B)complete{
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db) {
        [db beginTransaction];
        __block NSInteger counter = 0;
        [dictArray enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull dict, NSUInteger idx, BOOL * _Nonnull stop) {
            @autoreleasepool {
                NSArray* uniqueKeys = [XWFMDBTool executeSelector:xw_uniqueKeysSelector forClass:cla];
                ...
                if(uniqueKeys.count > 1){
                    ...
                    [uniqueKeys enumerateObjectsUsingBlock:^(NSString*  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop){
                        NSString* uniqueKey = xw_sqlKey(obj);
                        id uniqueKeyVlaue = tempDict[uniqueKey];
                        if(idx < (uniqueKeys.count-1)){
                            [where appendFormat:@" %@=%@ or",uniqueKey,xw_sqlValue(uniqueKeyVlaue)];
                        }else{
                            [where appendFormat:@" %@=%@",uniqueKey,xw_sqlValue(uniqueKeyVlaue)];
                        }
                        [tempDict removeObjectForKey:uniqueKey];
                    }];
                }else if(uniqueKeys.count == 1){
                    NSString* uniqueKey = xw_sqlKey([uniqueKeys firstObject]);
                    id uniqueKeyVlaue = tempDict[uniqueKey];
                    [where appendFormat:@" where %@=%@",uniqueKey,xw_sqlValue(uniqueKeyVlaue)];
                    [tempDict removeObjectForKey:uniqueKey];
                }else if([dict.allKeys containsObject:xw_sqlKey(xw_primaryKey)]){
                    NSString* primaryKey = xw_sqlKey(xw_primaryKey);
                    id primaryKeyVlaue = tempDict[primaryKey];
                    [where appendFormat:@" where %@=%@",primaryKey,xw_sqlValue(primaryKeyVlaue)];
    ...
    //数据监听执行函数
    [self doChangeWithName:name flag:result state:xw_update];
    xw_completeBlock(result);
}
...
/**
 批量插入或更新.
 */
-(void)xw_saveOrUpdateWithTableName:(NSString* _Nonnull)tablename class:(__unsafe_unretained _Nonnull Class)cla DictArray:(NSArray<NSDictionary*>* _Nonnull)dictArray complete:(xw_complete_B)complete{
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db) {
        [db beginTransaction];
        __block NSInteger counter = 0;
        [dictArray enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull dict, NSUInteger idx, BOOL * _Nonnull stop) {
            @autoreleasepool {
                ...
                NSArray* uniqueKeys = [XWFMDBTool executeSelector:xw_uniqueKeysSelector forClass:cla];
                //获得"联合主键"
                NSArray* unionPrimaryKeys =[XWFMDBTool executeSelector:xw_unionPrimaryKeysSelector forClass:cla];
                NSMutableDictionary* tempDict = [[NSMutableDictionary alloc] initWithDictionary:dict];
                NSMutableString* where = [NSMutableString new];
                ...
                if(uniqueKeys.count || unionPrimaryKeys.count){
                    ...
                    if(tempKeys.count == 1){
                        NSString* tempkey = xw_sqlKey([tempKeys firstObject]);
                        id tempkeyVlaue = tempDict[tempkey];
                        [where appendFormat:@" where %@=%@",tempkey,xw_sqlValue(tempkeyVlaue)];
                    }else{
                        [where appendString:@" where"];
                        [tempKeys enumerateObjectsUsingBlock:^(NSString*  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop){
                            NSString* tempkey = xw_sqlKey(obj);
                            id tempkeyVlaue = tempDict[tempkey];
                            if(idx < (tempKeys.count-1)){
                                [where appendFormat:@" %@=%@ %@",tempkey,xw_sqlValue(tempkeyVlaue),orAnd];
                            }else{
                                [where appendFormat:@" %@=%@",tempkey,xw_sqlValue(tempkeyVlaue)];
                            }
                        }];
                    }
                    ...
                    if([tempDict.allKeys containsObject:xw_id]){
                        //更新操作
                        id primaryKeyVlaue = tempDict[xw_id];
                        [where appendFormat:@" where %@=%@",xw_id,xw_sqlValue(primaryKeyVlaue)];
                    }else{
                        //插入操作
                        isSave = YES;
                    }
                ...
                NSMutableArray* arguments = [NSMutableArray array];
                if(isSave){//存储操作
                    NSInteger num = [self getKeyMaxForTable:tablename key:xw_id db:db];
                    [tempDict setValue:@(num+1) forKey:xw_id];
                    ...
                    if([tempDict.allKeys containsObject:xw_id]){
                        [tempDict removeObjectForKey:xw_id];//移除主键
                    }
                    [tempDict removeObjectForKey:xw_sqlKey(xw_createTimeKey)];//移除创建时间
    ...
    //数据监听执行函数
    [self doChangeWithName:tablename flag:result state:xw_update];
    xw_completeBlock(result);
}

-(void)queryQueueWithTableName:(NSString* _Nonnull)name conditions:(NSString* _Nullable)conditions complete:(xw_complete_A)complete{
    NSAssert(name,@"表名不能为空!");
    ...
    [self executeDB:^(FMDatabase * _Nonnull db){
        ...
        if (rs == nil) {
            xw_debug(@"查询错误,可能是'类变量名'发生了改变或'字段','表格'不存在!,请存储后再读取!");
        }else{
    ...
    xw_completeBlock(arrM);
}

/**
 直接传入条件sql语句查询
 */
-(void)queryWithTableName:(NSString* _Nonnull)name conditions:(NSString* _Nullable)conditions complete:(xw_complete_A)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
...
/**
 根据条件查询字段.
 */
-(void)queryWithTableName:(NSString* _Nonnull)name keys:(NSArray<NSString*>* _Nullable)keys where:(NSArray* _Nullable)where complete:(xw_complete_A)complete{
    NSAssert(name,@"表名不能为空!");
    ...
    [self executeDB:^(FMDatabase * _Nonnull db) {
        ...
        if(where && (where.count>0)){
            NSArray* results = [XWFMDBTool where:where];
        ...
        if (rs == nil) {
            xw_debug(@"查询错误,可能是'类变量名'发生了改变或'字段','表格'不存在!,请存储后再读取,或检查条件数组'字段名称'是否正确");
        }
        // 2.遍历结果集
        while (rs.next) {
            NSMutableDictionary* dictM = [[NSMutableDictionary alloc] init];
...
/**
 查询对象.
 */
-(void)queryWithTableName:(NSString* _Nonnull)name where:(NSString* _Nullable)where complete:(xw_complete_A)complete{
    NSAssert(name,@"表名不能为空!");
    NSMutableArray* arrM = [[NSMutableArray alloc] init];
    [self executeDB:^(FMDatabase * _Nonnull db) {
...
/**
 更新数据.
 */
-(void)updateWithTableName:(NSString* _Nonnull)name valueDict:(NSDictionary* _Nonnull)valueDict where:(NSArray* _Nullable)where complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
...
-(void)updateQueueWithTableName:(NSString* _Nonnull)name valueDict:(NSDictionary* _Nullable)valueDict conditions:(NSString* _Nonnull)conditions complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db){
        NSString* SQL;
...
/**
 直接传入条件sql语句更新.
 */
-(void)updateWithObject:(id _Nonnull)object valueDict:(NSDictionary* _Nullable)valueDict conditions:(NSString* _Nonnull)conditions complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
        //自动判断是否有字段改变,自动刷新数据库.
        [self ifIvarChangeForObject:object ignoredKeys:[XWFMDBTool executeSelector:xw_ignoreKeysSelector forClass:[object class]]];
...
/**
 直接传入条件sql语句更新对象.
 */
-(void)updateObject:(id _Nonnull)object ignoreKeys:(NSArray* const _Nullable)ignoreKeys conditions:(NSString* _Nonnull)conditions complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
        NSString* tableName = [XWFMDBTool getTableNameWithObject:object];
        //自动判断是否有字段改变,自动刷新数据库.
        [self ifIvarChangeForObject:object ignoredKeys:ignoreKeys];
        NSDictionary* valueDict = [XWFMDBTool getDictWithObject:self ignoredKeys:ignoreKeys filtModelInfoType:xw_ModelInfoSingleUpdate];
        [self updateQueueWithTableName:tableName valueDict:valueDict conditions:conditions complete:complete];
    }
    dispatch_semaphore_signal(self.semaphore);
}
/**
 根据keypath更新数据
 */
-(void)updateWithTableName:(NSString* _Nonnull)name forKeyPathAndValues:(NSArray* _Nonnull)keyPathValues valueDict:(NSDictionary* _Nonnull)valueDict complete:(xw_complete_B)complete{
    NSString* like = [XWFMDBTool getLikeWithKeyPathAndValues:keyPathValues where:YES];
    NSMutableArray* arguments = [NSMutableArray array];
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db){
        NSMutableString* SQL = [[NSMutableString alloc] init];
...
/**
 根据条件删除数据.
 */
-(void)deleteWithTableName:(NSString* _Nonnull)name where:(NSArray* _Nonnull)where complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
    ...
    //数据监听执行函数
    [self doChangeWithName:name flag:result state:xw_delete];
    xw_completeBlock(result);
}

-(void)deleteQueueWithTableName:(NSString* _Nonnull)name conditions:(NSString* _Nullable)conditions complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db) {
...
/**
 直接传入条件sql语句删除.
 */
-(void)deleteWithTableName:(NSString* _Nonnull)name conditions:(NSString* _Nullable)conditions complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    [self deleteQueueWithTableName:name conditions:conditions complete:complete];
    dispatch_semaphore_signal(self.semaphore);
}

-(void)deleteQueueWithTableName:(NSString* _Nonnull)name forKeyPathAndValues:(NSArray* _Nonnull)keyPathValues complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
    NSString* like = [XWFMDBTool getLikeWithKeyPathAndValues:keyPathValues where:YES];
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db) {
...
//根据keypath删除表内容.
-(void)deleteWithTableName:(NSString* _Nonnull)name forKeyPathAndValues:(NSArray* _Nonnull)keyPathValues complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
...
/**
 根据表名删除表格全部内容.
 */
-(void)clearTable:(NSString* _Nonnull)name complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db) {
...
/**
 删除表.
 */
-(void)dropTable:(NSString* _Nonnull)name complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db) {
    ...
    //数据监听执行函数
    [self doChangeWithName:name flag:result state:xw_drop];
    xw_completeBlock(result);
}
...
/**
 删除表(线程安全).
 */
-(void)dropSafeTable:(NSString* _Nonnull)name complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
...
/**
 动态添加表字段.
 */
-(void)addTable:(NSString* _Nonnull)name key:(NSString* _Nonnull)key complete:(xw_complete_B)complete{
    NSAssert(name,@"表名不能为空!");
    __block BOOL result;
    [self executeDB:^(FMDatabase * _Nonnull db) {
        NSString* SQL = [NSString stringWithFormat:@"alter table %@ add %@;",name,[XWFMDBTool keyAndType:key]];
        xw_debug(SQL);
        result = [db executeUpdate:SQL];
    }];
    xw_completeBlock(result);
...
-(NSInteger)countQueueForTable:(NSString* _Nonnull)name where:(NSArray* _Nullable)where{
    ...
    for(int i=0;i<where.count;i+=3){
        if ([where[i+2] isKindOfClass:[NSString class]]) {
            [strM appendFormat:@"%@%@%@'%@'",XW,where[i],where[i+1],where[i+2]];
        }else{
            [strM appendFormat:@"%@%@%@%@",XW,where[i],where[i+1],where[i+2]];
        }
 ...
 */
-(double)sqliteMethodQueueForTable:(NSString* _Nonnull)name type:(xw_sqliteMethodType)methodType key:(NSString*)key where:(NSString* _Nullable)where{
    NSAssert(name,@"表名不能为空!");
    ...
    switch (methodType) {
        case xw_min:
            method = [NSString stringWithFormat:@"min(%@%@)",XW,key];
            break;
        case xw_max:
            method = [NSString stringWithFormat:@"max(%@%@)",XW,key];
            break;
        case xw_sum:
            method = [NSString stringWithFormat:@"sum(%@%@)",XW,key];
            break;
        case xw_avg:
            method = [NSString stringWithFormat:@"avg(%@%@)",XW,key];
            break;
        default:
            NSAssert(NO,@"请传入方法类型!");
            break;
...
/**
 直接调用sqliteb的原生函数计算sun,min,max,avg等.
 */
-(double)sqliteMethodForTable:(NSString* _Nonnull)name type:(xw_sqliteMethodType)methodType key:(NSString*)key where:(NSString* _Nullable)where{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    double num = 0.0;
    @autoreleasepool {
...
-(void)copyA:(NSString* _Nonnull)A toB:(NSString* _Nonnull)B class:(__unsafe_unretained _Nonnull Class)cla keys:(NSArray<NSString*>* const _Nonnull)keys complete:(xw_complete_I)complete{
    //获取"唯一约束"字段名
    NSArray* uniqueKeys = [XWFMDBTool executeSelector:xw_uniqueKeysSelector forClass:cla];
    //获取“联合主键”字段名
    NSArray* unionPrimaryKeys = [XWFMDBTool executeSelector:xw_unionPrimaryKeysSelector forClass:cla];
    ...
    __block xw_dealState refreshstate = xw_error;
    __block BOOL recordError = NO;
    __block BOOL recordSuccess = NO;
    __weak typeof(self) XWSelf = self;
            ...
            [self queryWithTableName:A where:param complete:^(NSArray * _Nullable array) {
                for(NSDictionary* oldDict in array){
                    ...
                    for(NSString* keyAndType in keys){
                        ...
                        //字段名前加上 @"XW_"
                        key = [NSString stringWithFormat:@"%@%@",XW,key];
                        if (oldDict[key]){
                            newDict[key] = oldDict[key];
                        }
                    }
                    //将旧表的数据插入到新表
                    [XWSelf insertIntoTableName:B Dict:newDict complete:^(BOOL isSuccess){
                        if (isSuccess){
                            if (!recordSuccess) {
                                recordSuccess = YES;
                            }
    ...
    if (complete){
        if (recordError && recordSuccess) {
            refreshstate = xw_incomplete;
        }else if(recordError && !recordSuccess){
            refreshstate = xw_error;
        }else if (recordSuccess && !recordError){
            refreshstate = xw_complete;
        }else;
...
-(void)refreshQueueTable:(NSString* _Nonnull)name class:(__unsafe_unretained _Nonnull Class)cla keys:(NSArray<NSString*>* const _Nonnull)keys complete:(xw_complete_I)complete{
    NSAssert(name,@"表名不能为空!");
    NSAssert(keys,@"字段数组不能为空!");
    [self isExistWithTableName:name complete:^(BOOL isSuccess){
        if (!isSuccess){
            xw_debug(@"没有数据存在,数据库更新失败!");
            xw_completeBlock(xw_error);
            return;
        }
    }];
    NSString* XWTempTable = @"XWTempTable";
    //事务操作.
    __block int recordFailCount = 0;
    [self executeTransation:^BOOL{
        [self copyA:name toB:XWTempTable class:cla keys:keys complete:^(xw_dealState result) {
            if(result == xw_complete){
                recordFailCount++;
            }
        }];
        ...
        [self copyA:XWTempTable toB:name class:cla keys:keys complete:^(xw_dealState result) {
            if(result == xw_complete){
                recordFailCount++;
            }
        }];
        [self dropTable:XWTempTable complete:^(BOOL isSuccess) {
            if(isSuccess)recordFailCount++;
        }];
        if(recordFailCount != 4){
            xw_debug(@"发生错误，更新数据库失败!");
        }
    ...
    if (recordFailCount==0) {
        ...
        xw_completeBlock(xw_incomplete);
    }else{
        xw_completeBlock(xw_complete);
    }
}
 ...
 */
-(void)refreshTable:(NSString* _Nonnull)name class:(__unsafe_unretained _Nonnull Class)cla keys:(NSArray<NSString*>* const _Nonnull)keys complete:(xw_complete_I)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
...
-(void)copyA:(NSString* _Nonnull)A toB:(NSString* _Nonnull)B keyDict:(NSDictionary* const _Nullable)keyDict complete:(xw_complete_I)complete{
    //获取"唯一约束"字段名
    NSArray* uniqueKeys = [XWFMDBTool executeSelector:xw_uniqueKeysSelector forClass:NSClassFromString(A)];
    //获取“联合主键”字段名
    NSArray* unionPrimaryKeys = [XWFMDBTool executeSelector:xw_unionPrimaryKeysSelector forClass:NSClassFromString(A)];
    __block NSArray* keys = [XWFMDBTool getClassIvarList:NSClassFromString(A) Object:nil onlyKey:NO];
    ...
    for(NSInteger i=0;i<count;i+=MaxQueryPageNum){
        @autoreleasepool{//由于查询出来的数据量可能巨大,所以加入自动释放池.
            NSString* param = [NSString stringWithFormat:@"limit %@,%@",@(i),@(MaxQueryPageNum)];
            [self queryWithTableName:A where:param complete:^(NSArray * _Nullable array) {
                __strong typeof(XWSelf) strongSelf = XWSelf;
                for(NSDictionary* oldDict in array){
                    NSMutableDictionary* newDict = [NSMutableDictionary dictionary];
                    for(NSString* keyAndType in keys){
                        NSString* key = [keyAndType componentsSeparatedByString:@"*"][0];
                    ...
                    for(int i=0;i<oldKeys.count;i++){
                        //字段名前加上 @"XW_"
                        NSString* oldkey = [NSString stringWithFormat:@"%@%@",XW,oldKeys[i]];
                        NSString* newkey = [NSString stringWithFormat:@"%@%@",XW,newKeys[i]];
                        if (oldDict[oldkey]){
...
-(void)refreshQueueTable:(NSString* _Nonnull)tablename class:(__unsafe_unretained _Nonnull Class)cla keys:(NSArray* const _Nonnull)keys keyDict:(NSDictionary* const _Nonnull)keyDict complete:(xw_complete_I)complete{
    ...
    __block int recordFailCount = 0;
    [self executeTransation:^BOOL{
        [self copyA:tablename toB:XWTempTable keyDict:keyDict complete:^(xw_dealState result) {
            if(result == xw_complete){
                recordFailCount++;
            }
        }];
        ...
        [self copyA:XWTempTable toB:tablename class:cla keys:keys complete:^(xw_dealState result) {
            if(result == xw_complete){
                recordFailCount++;
            }
        }];
...
-(void)refreshTable:(NSString* _Nonnull)name class:(__unsafe_unretained _Nonnull Class)cla keys:(NSArray* const _Nonnull)keys keyDict:(NSDictionary* const _Nonnull)keyDict complete:(xw_complete_I)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
...
-(void)ifIvarChangeForObject:(id)object ignoredKeys:(NSArray*)ignoredkeys{
    ...
    @autoreleasepool {
        ...
        [self executeDB:^(FMDatabase * _Nonnull db) {
            ...
            NSMutableArray* keys = [NSMutableArray arrayWithArray:[XWFMDBTool getClassIvarList:[object class] Object:nil onlyKey:YES]];
            if (ignoredkeys) {
            ...
            [columNames enumerateObjectsUsingBlock:^(NSString* _Nonnull columName, NSUInteger idx, BOOL * _Nonnull stop) {
                NSString* propertyName = [columName stringByReplacingOccurrencesOfString:XW withString:@""];
        ...
        if((sqlKeys.count==0) && (newKeys.count>0)){
            ...
            NSMutableArray* newTableKeys = [[NSMutableArray alloc] initWithArray:[XWFMDBTool getClassIvarList:[object class] Object:nil onlyKey:NO]];
...
/**
 处理插入的字典数据并返回
 */
-(void)insertWithObject:(id)object ignoredKeys:(NSArray* const _Nullable)ignoredKeys complete:(xw_complete_B)complete{
    NSDictionary* dictM = [XWFMDBTool getDictWithObject:object ignoredKeys:ignoredKeys filtModelInfoType:xw_ModelInfoInsert];
    //自动判断是否有字段改变,自动刷新数据库.
...
-(NSArray*)getArray:(NSArray*)array ignoredKeys:(NSArray* const _Nullable)ignoredKeys filtModelInfoType:(xw_getModelInfoType)filtModelInfoType{
    NSMutableArray* dictArray = [NSMutableArray array];
    [array enumerateObjectsUsingBlock:^(id  _Nonnull object, NSUInteger idx, BOOL * _Nonnull stop) {
        NSDictionary* dict = [XWFMDBTool getDictWithObject:object ignoredKeys:ignoredKeys filtModelInfoType:filtModelInfoType];
...
/**
 批量插入数据
 */
-(void)insertWithObjects:(NSArray*)array ignoredKeys:(NSArray* const _Nullable)ignoredKeys complete:(xw_complete_B)complete{
    NSArray* dictArray = [self getArray:array ignoredKeys:ignoredKeys filtModelInfoType:xw_ModelInfoInsert];
    //自动判断是否有字段改变,自动刷新数据库.
    [self ifIvarChangeForObject:array.firstObject ignoredKeys:ignoredKeys];
    NSString* tableName = [XWFMDBTool getTableNameWithObject:array.firstObject];
...
/**
 ...
 over
 */
-(void)updateSetWithObjects:(NSArray*)array ignoredKeys:(NSArray* const _Nullable)ignoredKeys complete:(xw_complete_B)complete{
    NSArray* dictArray = [self getArray:array ignoredKeys:ignoredKeys filtModelInfoType:xw_ModelInfoArrayUpdate];
    NSString* tableName = [XWFMDBTool getTableNameWithObject:array.firstObject];
...
/**
 批量存储.
 */
-(void)saveObjects:(NSArray* _Nonnull)array ignoredKeys:(NSArray* const _Nullable)ignoredKeys complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
        [XWFMDBTool ifNotExistWillCreateTableWithObject:array.firstObject ignoredKeys:ignoredKeys];
...
/**
 ...
 over
 */
-(void)updateObjects:(NSArray* _Nonnull)array ignoredKeys:(NSArray* const _Nullable)ignoredKeys complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
...
/**
 批量插入或更新.
 */
-(void)xw_saveOrUpateArray:(NSArray* _Nonnull)array ignoredKeys:(NSArray* const _Nullable)ignoredKeys complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
        //判断是否建表.
        [XWFMDBTool ifNotExistWillCreateTableWithObject:array.firstObject ignoredKeys:ignoredKeys];
        ...
        [self ifIvarChangeForObject:array.firstObject ignoredKeys:ignoredKeys];
        //转换模型数据
        NSArray* dictArray = [self getArray:array ignoredKeys:ignoredKeys filtModelInfoType:xw_ModelInfoNone];
        //获取自定义表名
        NSString* tableName = [XWFMDBTool getTableNameWithObject:array.firstObject];
        [self xw_saveOrUpdateWithTableName:tableName class:[array.firstObject class] DictArray:dictArray complete:complete];
    }
    dispatch_semaphore_signal(self.semaphore);
}
...
/**
 存储一个对象.
 */
-(void)saveObject:(id _Nonnull)object ignoredKeys:(NSArray* const _Nullable)ignoredKeys complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
        [XWFMDBTool ifNotExistWillCreateTableWithObject:object ignoredKeys:ignoredKeys];
        [self insertWithObject:object ignoredKeys:ignoredKeys complete:complete];
    }
    dispatch_semaphore_signal(self.semaphore);
}

-(void)queryObjectQueueWithTableName:(NSString* _Nonnull)tablename class:(__unsafe_unretained _Nonnull Class)cla where:(NSString* _Nullable)where complete:(xw_complete_A)complete{
    //检查是否建立了跟对象相对应的数据表
    __weak typeof(self) XWSelf = self;
    [self isExistWithTableName:tablename complete:^(BOOL isExist) {
        __strong typeof(XWSelf) strongSelf = XWSelf;
        if (!isExist){//如果不存在就返回空
            xw_completeBlock(nil);
        }else{
            [strongSelf queryWithTableName:tablename where:where complete:^(NSArray * _Nullable array) {
                NSArray* resultArray = [XWFMDBTool tansformDataFromSqlDataWithTableName:tablename class:cla array:array];
                xw_completeBlock(resultArray);
            }];
        }
    }];
}
/**
 查询对象.
 */
-(void)queryObjectWithTableName:(NSString* _Nonnull)tablename class:(__unsafe_unretained _Nonnull Class)cla where:(NSString* _Nullable)where complete:(xw_complete_A)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
...
-(void)updateQueueWithObject:(id _Nonnull)object where:(NSArray* _Nullable)where ignoreKeys:(NSArray* const _Nullable)ignoreKeys complete:(xw_complete_B)complete{
    NSDictionary* valueDict = [XWFMDBTool getDictWithObject:object ignoredKeys:ignoreKeys filtModelInfoType:xw_ModelInfoSingleUpdate];
    NSString* tableName = [XWFMDBTool getTableNameWithObject:object];
...
/**
 根据条件改变对象数据.
 */
-(void)updateWithObject:(id _Nonnull)object where:(NSArray* _Nullable)where ignoreKeys:(NSArray* const _Nullable)ignoreKeys complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
...
-(void)updateQueueWithObject:(id _Nonnull)object forKeyPathAndValues:(NSArray* _Nonnull)keyPathValues ignoreKeys:(NSArray* const _Nullable)ignoreKeys complete:(xw_complete_B)complete{
    NSDictionary* valueDict = [XWFMDBTool getDictWithObject:object ignoredKeys:ignoreKeys filtModelInfoType:xw_ModelInfoSingleUpdate];
    NSString* tableName = [XWFMDBTool getTableNameWithObject:object];
    __weak typeof(self) XWSelf = self;
    [self isExistWithTableName:tableName complete:^(BOOL isExist){
...
/**
 根据keyPath改变对象数据.
 */
-(void)updateWithObject:(id _Nonnull)object forKeyPathAndValues:(NSArray* _Nonnull)keyPathValues ignoreKeys:(NSArray* const _Nullable)ignoreKeys complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
        //自动判断是否有字段改变,自动刷新数据库.
        [self ifIvarChangeForObject:object ignoredKeys:ignoreKeys];
...
/**
 根据类删除此类所有表数据.
 */
-(void)clearWithObject:(id _Nonnull)object complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
...
/**
 根据类,删除这个类的表.
 */
-(void)dropWithTableName:(NSString* _Nonnull)tablename complete:(xw_complete_B)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
...
-(void)copyQueueTable:(NSString* _Nonnull)srcTable to:(NSString* _Nonnull)destTable keyDict:(NSDictionary* const _Nonnull)keydict append:(BOOL)append complete:(xw_complete_I)complete{
    ...
    [self isExistWithTableName:destTable complete:^(BOOL isExist) {
        if(!isExist){
            ...
            if (!append){//覆盖模式,即将原数据删掉,拷贝新的数据过来
                [XWSelf clearTable:destTable complete:nil];
            }
        }
    }];
    __block xw_dealState copystate = xw_error;
    __block BOOL recordError = NO;
    __block BOOL recordSuccess = NO;
            ...
            [self queryWithTableName:srcTable where:param complete:^(NSArray * _Nullable array) {
                for(NSDictionary* srcDict in array){
                    ...
                    for(int i=0;i<srcKeys.count;i++){
                        //字段名前加上 @"XW_"
                        NSString* destSqlKey = [NSString stringWithFormat:@"%@%@",XW,destKeys[i]];
                        NSString* srcSqlKey = [NSString stringWithFormat:@"%@%@",XW,srcKeys[i]];
                        destDict[destSqlKey] = srcDict[srcSqlKey];
                    }
                    [XWSelf insertIntoTableName:destTable Dict:destDict complete:^(BOOL isSuccess) {
                        if (isSuccess){
                            if (!recordSuccess) {
                                recordSuccess = YES;
                            }
    ...
    if (complete){
        if (recordError && recordSuccess) {
            copystate = xw_incomplete;
        }else if(recordError && !recordSuccess){
            copystate = xw_error;
        }else if (recordSuccess && !recordError){
            copystate = xw_complete;
        }else;
        complete(copystate);
    }
...
/**
 将某表的数据拷贝给另一个表
 */
-(void)copyTable:(NSString* _Nonnull)srcTable to:(NSString* _Nonnull)destTable keyDict:(NSDictionary* const _Nonnull)keydict append:(BOOL)append complete:(xw_complete_I)complete{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
...
/**
 ...
 @cla 要操作的类.
 */
-(id _Nullable)xw_executeSql:(NSString* const _Nonnull)sql tablename:(NSString* _Nonnull)tablename class:(__unsafe_unretained _Nonnull Class)cla{
    ...
    [self executeDB:^(FMDatabase * _Nonnull db){
        if([[sql lowercaseString] hasPrefix:@"select"]){
            ...
            [rs close];
            //转换结果
            result = [XWFMDBTool tansformDataFromSqlDataWithTableName:tablename class:cla array:result];
        }else{
            result = @([db executeUpdate:sql]);
        }
        xw_debug(sql);
...
/**
 直接存储数组.
 */
-(void)saveArray:(NSArray* _Nonnull)array name:(NSString*)name complete:(xw_complete_B)complete{
    ...
    @autoreleasepool {
        ...
        [self isExistWithTableName:name complete:^(BOOL isSuccess) {
            if (!isSuccess) {
                [XWSelf createTableWithTableName:name keys:@[[NSString stringWithFormat:@"%@*i",xw_primaryKey],@"param*@\"NSString\"",@"index*i"] unionPrimaryKeys:nil uniqueKeys:nil complete:nil];
            }
        }];
        ...
        [self executeTransation:^BOOL{
            for(id value in array){
                ...
                id sqlValue = [XWFMDBTool getSqlValue:value type:type encode:YES];
                sqlValue = [NSString stringWithFormat:@"%@$$$%@",sqlValue,type];
                NSDictionary* dict = @{@"XW_param":sqlValue,@"XW_index":@(sqlCount++)};
        ...
        xw_completeBlock(array.count==num);
    }
    dispatch_semaphore_signal(self.semaphore);
}
/**
 读取数组.
 */
-(void)queryArrayWithName:(NSString*)name complete:(xw_complete_A)complete{
    NSAssert(name,@"唯一标识名不能为空!");
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    @autoreleasepool {
        NSString* condition = [NSString stringWithFormat:@"order by %@ asc",xw_sqlKey(xw_primaryKey)];
        [self queryQueueWithTableName:name conditions:condition complete:^(NSArray * _Nullable array) {
            ...
            if(array&&array.count){
                ...
                for(NSDictionary* dict in array){
                    NSArray* keyAndTypes = [dict[@"XW_param"] componentsSeparatedByString:@"$$$"];
                    id value = [keyAndTypes firstObject];
                    NSString* type = [keyAndTypes lastObject];
                    value = [XWFMDBTool getSqlValue:value type:type encode:NO];
                    [resultM addObject:value];
                }
            }
            xw_completeBlock(resultM);
        }];
    }
    dispatch_semaphore_signal(self.semaphore);
}
...
-(id _Nullable)queryArrayWithName:(NSString* _Nonnull)name index:(NSInteger)index{
    ...
    @autoreleasepool {
        [self queryQueueWithTableName:name conditions:[NSString stringWithFormat:@"where XW_index=%@",@(index)] complete:^(NSArray * _Nullable array){
            if(array&&array.count){
                NSDictionary* dict = [array firstObject];
                NSArray* keyAndTypes = [dict[@"XW_param"] componentsSeparatedByString:@"$$$"];
                id value = [keyAndTypes firstObject];
                NSString* type = [keyAndTypes lastObject];
                resultValue = [XWFMDBTool getSqlValue:value type:type encode:NO];
            }
        }];
    }
    dispatch_semaphore_signal(self.semaphore);
...
-(BOOL)updateObjectWithName:(NSString* _Nonnull)name object:(id _Nonnull)object index:(NSInteger)index{
    ...
    @autoreleasepool{
        ...
        id sqlValue = [XWFMDBTool getSqlValue:object type:type encode:YES];
        sqlValue = [NSString stringWithFormat:@"%@$$$%@",sqlValue,type];
        NSDictionary* dict = @{@"XW_param":sqlValue};
...
-(BOOL)deleteObjectWithName:(NSString* _Nonnull)name index:(NSInteger)index{
    ...
    @autoreleasepool {
        [self executeTransation:^BOOL{
            [self deleteQueueWithTableName:name conditions:[NSString stringWithFormat:@"where XW_index=%@",@(index)] complete:^(BOOL isSuccess) {
                if(isSuccess) {
                    flag++;
                }
            }];
            if(flag){
                [self updateQueueWithTableName:name valueDict:nil conditions:[NSString stringWithFormat:@"set XW_index=XW_index-1 where XW_index>%@",@(index)] complete:^(BOOL isSuccess) {
                    if(isSuccess) {
                        flag++;
                    }
                }];
...
/**
 直接存储字典.
 */
-(void)saveDictionary:(NSDictionary* _Nonnull)dictionary complete:(xw_complete_B)complete{
    ...
    @autoreleasepool {
        ...
        [self isExistWithTableName:tableName complete:^(BOOL isSuccess) {
            if (!isSuccess) {
                [XWSelf createTableWithTableName:tableName keys:@[[NSString stringWithFormat:@"%@*i",xw_primaryKey],@"key*@\"NSString\"",@"value*@\"NSString\""] unionPrimaryKeys:nil uniqueKeys:@[@"key"] complete:nil];
            }
        }];
        __block NSInteger num = 0;
        [self executeTransation:^BOOL{
            [dictionary enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull value, BOOL * _Nonnull stop){
                ...
                id sqlValue = [XWFMDBTool getSqlValue:value type:type encode:YES];
                sqlValue = [NSString stringWithFormat:@"%@$$$%@",sqlValue,type];
                NSDictionary* dict = @{@"XW_key":key,@"XW_value":sqlValue};
        ...
        xw_completeBlock(dictionary.allKeys.count==num);
    }
    dispatch_semaphore_signal(self.semaphore);
}
/**
 添加字典元素.
 */
-(BOOL)xw_setValue:(id _Nonnull)value forKey:(NSString* const _Nonnull)key{
...
/**
 更新字典元素.
 */
-(BOOL)xw_updateValue:(id _Nonnull)value forKey:(NSString* const _Nonnull)key{
    NSAssert(key,@"key不能为空!");
    NSAssert(value,@"value不能为空!");
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    __block BOOL result;
    @autoreleasepool{
        ...
        id sqlvalue = [XWFMDBTool getSqlValue:value type:type encode:YES];
        sqlvalue = [NSString stringWithFormat:@"%@$$$%@",sqlvalue,type];
        NSDictionary* dict = @{@"XW_value":sqlvalue};
        NSString* const tableName = @"XW_Dictionary";
...
-(void)xw_enumerateKeysAndObjectsUsingBlock:(void (^ _Nonnull)(NSString* _Nonnull key, id _Nonnull value,BOOL *stop))block{
    ...
    @autoreleasepool{
        ...
        [self queryQueueWithTableName:tableName conditions:condition complete:^(NSArray * _Nullable array) {
            BOOL stopFlag = NO;
            for(NSDictionary* dict in array){
                NSArray* keyAndTypes = [dict[@"XW_value"] componentsSeparatedByString:@"$$$"];
                NSString* key = dict[@"XW_key"];
                id value = [keyAndTypes firstObject];
                NSString* type = [keyAndTypes lastObject];
                value = [XWFMDBTool getSqlValue:value type:type encode:NO];
...
-(id _Nullable)xw_valueForKey:(NSString* const _Nonnull)key{
    ...
    @autoreleasepool {
        NSString* const tableName = @"XW_Dictionary";
        [self queryQueueWithTableName:tableName conditions:[NSString stringWithFormat:@"where XW_key='%@'",key] complete:^(NSArray * _Nullable array){
            if(array&&array.count){
                NSDictionary* dict = [array firstObject];
                NSArray* keyAndTypes = [dict[@"XW_value"] componentsSeparatedByString:@"$$$"];
                id value = [keyAndTypes firstObject];
...
/**
 删除字典元素.
 */
-(BOOL)xw_deleteValueForKey:(NSString* const _Nonnull)key{
    NSAssert(key,@"key不能为空!");
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    __block BOOL result;
    @autoreleasepool {
        NSString* const tableName = @"XW_Dictionary";
        [self deleteQueueWithTableName:tableName conditions:[NSString stringWithFormat:@"where XW_key='%@'",key] complete:^(BOOL isSuccess) {
            result = isSuccess;
        }];
    }
    dispatch_semaphore_signal(self.semaphore);
