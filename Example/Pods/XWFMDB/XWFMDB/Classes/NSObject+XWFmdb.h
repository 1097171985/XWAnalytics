 ...
 */
+(NSArray* _Nonnull)xw_unionPrimaryKeys;
/**
 自定义 “唯一约束” 函数,如果需要 “唯一约束”字段,则在类中自己实现该函数.
 @return 返回值是 “唯一约束” 的字段名(即相对应的变量名).
 */
+(NSArray* _Nonnull)xw_uniqueKeys;
/**
 @return 返回不需要存储的属性.
 */
+(NSArray* _Nonnull)xw_ignoreKeys;
/**
 *  数组中需要转换的模型类(‘字典转模型’ 或 ’模型转字典‘ 都需要实现该函数)
 *  @return 字典中的key是数组属性名，value是数组中存放模型的Class
 */
+(NSDictionary *_Nonnull)xw_objectClassInArray;
/**
 如果模型中有自定义类变量,则实现该函数对应进行集合到模型的转换.
 字典转模型用.
 */
+(NSDictionary *_Nonnull)xw_objectClassForCustom;
/**
 将模型中对应的自定义类变量转换为字典.
 模型转字典用.
 */
+(NSDictionary *_Nonnull)xw_dictForCustomClass;
/**
 替换变量的功能(及当字典的key和属性名不一样时，进行映射对应起来)
 */
+(NSDictionary *_Nonnull)xw_replacedKeyFromPropertyName;

@end


@interface NSObject (XWFmdb)<XWProtocol>
/**
 本库自带的自动增长主键.
 */
@property(nonatomic,strong)NSNumber* _Nonnull xw_id;
/**
 为了方便开发者，特此加入以下两个字段属性供开发者做参考.(自动记录数据的存入时间和更新时间)
 数据创建时间(即存入数据库的时间)
 */
@property(nonatomic,copy)NSString* _Nonnull xw_createTime;
/**
 为了方便开发者，特此加入以下两个字段属性供开发者做参考.(自动记录数据的存入时间和更新时间)
 数据最后那次更新的时间.
 */
@property(nonatomic,copy)NSString* _Nonnull xw_updateTime;
/**
 自定义表名
 */
@property(nonatomic,copy)NSString* _Nonnull xw_tableName;
/**
 @tablename 此参数为nil时，判断以当前类名为表名的表是否存在; 此参数非nil时,判断以当前参数为表名的表是否存在.
 */
+(BOOL)xw_isExistForTableName:(NSString* _Nullable)tablename;
/**
 同步存储.
 */
-(BOOL)xw_save;
 ...
 */
+(id _Nonnull)xw_objectWithKeyValues:(id const _Nonnull)keyValues;
+(id _Nonnull)xw_objectWithDictionary:(NSDictionary* const _Nonnull)dictionary;
/**
 直接传数组批量处理;
 注:array中的元素是字典,否则出错.
 */
 ...
 */
-(NSMutableDictionary* _Nonnull)xw_keyValuesIgnoredKeys:(NSArray* _Nullable)ignoredKeys;

#warning mark 过期方法(能正常使用,但不建议使用)
/**
 判断这个类的数据表是否已经存在.
 */
+(BOOL)xw_isExist XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，使用xw_isExistForTableName:替代.");
/**
 同步存入对象数组.
 @array 存放对象的数组.(数组中存放的是同一种类型的数据)
 */
+(BOOL)xw_saveArray:(NSArray* _Nonnull)array XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，请使用xw_saveOrUpdateArray替代");
/**
 同上条件异步.
 */
+(void)xw_saveArrayAsync:(NSArray* _Nonnull)array complete:(xw_complete_B)complete XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，请使用xw_saveOrUpdateArray替代");
...
/**
 ...
 提示：“唯一约束”优先级高于"主键".
 */
+(BOOL)xw_updateArray:(NSArray* _Nonnull)array XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，请使用xw_saveOrUpdateArray替代");
/**
 同上条件异步.
 */
+(void)xw_updateArrayAsync:(NSArray* _Nonnull)array complete:(xw_complete_B)complete XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，请使用xw_saveOrUpdateArray替代");
/**
 同步存入对象数组.
 @array 存放对象的数组.(数组中存放的是同一种类型的数据)
 */
+(BOOL)xw_saveArray:(NSArray* _Nonnull)array IgnoreKeys:(NSArray* const _Nullable)ignoreKeys XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，在模型的.m文件中实现xw_ignoreKeys函数即可");
/**
 异步存入对象数组.
 @array 存放对象的数组.(数组中存放的是同一种类型的数据)
 */
+(void)xw_saveArrayAsync:(NSArray* _Nonnull)array IgnoreKeys:(NSArray* const _Nullable)ignoreKeys complete:(xw_complete_B)complete XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，在模型的.m文件中实现xw_ignoreKeys函数即可");
/**
 同步存储.
 @ignoreKeys 忽略掉模型中的哪些key(即模型变量)不要存储.
 */
-(BOOL)xw_saveIgnoredKeys:(NSArray* const _Nonnull)ignoredKeys XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，在模型的.m文件中实现xw_ignoreKeys函数即可");
/**
 异步存储.
 @ignoreKeys 忽略掉模型中的哪些key(即模型变量)不要存储.
 */
-(void)xw_saveAsyncIgnoreKeys:(NSArray* const _Nonnull)ignoredKeys complete:(xw_complete_B)complete XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，在模型的.m文件中实现xw_ignoreKeys函数即可");
/**
 同步覆盖存储.
 覆盖掉原来的数据,只存储当前的数据.
 @ignoreKeys 忽略掉模型中的哪些key(即模型变量)不要存储.
 */
-(BOOL)xw_coverIgnoredKeys:(NSArray* const _Nonnull)ignoredKeys XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，在模型的.m文件中实现xw_ignoreKeys函数即可");
/**
 异步覆盖存储.
 覆盖掉原来的数据,只存储当前的数据.
 @ignoreKeys 忽略掉模型中的哪些key(即模型变量)不要存储.
 */
-(void)xw_coverAsyncIgnoredKeys:(NSArray* const _Nonnull)ignoredKeys complete:(xw_complete_B)complete XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，在模型的.m文件中实现xw_ignoreKeys函数即可");
/**
 同步更新数据.
 ...
 */
-(BOOL)xw_updateWhere:(NSArray* _Nullable)where ignoreKeys:(NSArray* const _Nullable)ignoreKeys XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，在模型的.m文件中实现xw_ignoreKeys函数即可");
/**
 ...
 @ignoreKeys 忽略哪些key不用更新.
 */
-(BOOL)xw_updateFormatSqlConditions:(NSString* _Nonnull)conditions IgnoreKeys:(NSArray* const _Nullable)ignoreKeys XWFMDBDeprecated("此方法已过期(能正常使用,但不建议使用)，在模型的.m文件中实现xw_ignoreKeys函数即可");
@end

#pragma mark 直接存储数组.
@interface NSArray (XWFmdb)
/**
 存储数组.
 @name 唯一标识名称.
 **/
...
/**
 查询整个数组
 */
+(NSArray* _Nullable)xw_arrayWithName:(NSString* const _Nonnull)name;
/**
 获取数组某个位置的元素.
 @name 唯一标识名称.
 @index 数组元素位置.
